...
> {
  ...
  private fetchResources = async () => {
    ...
    const responseLocation = await fetch(
      `${apiPath}/location/location-by-email`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  public filterResourcesByNameOrDescription = () => {
    ...
    if (textElement) {
      ...
      this.setState({
        services: this.state.serviceNotFiltered.filter((service) =>
          service.name.includes(text)
        ),
      });
      this.setState({
        location: this.state.locationNotFiltered.filter((location) =>
  ...
  public filterResourcesByPrice = () => {
    ...
    if (priceElement) {
      ...
      this.setState({
        services: this.state.serviceNotFiltered.filter(
          (service) => service.price <= price
        ),
      });
      this.setState({
        location: this.state.locationNotFiltered.filter(
  ...
  render() {
    if (
      this.state.serviceNotFiltered.length === 0 ||
      this.state.locationNotFiltered.length === 0
    ) {
      return <Navbar />;
    }
